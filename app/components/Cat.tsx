 ...
 * refresh. Falls back to a CSS-drawn cat ONLY if the sheet genuinely 404s.
...
// ---------------------------------------------------------------------------
// CSS fallback (only used if the sprite sheet 404s).
// ---------------------------------------------------------------------------
...
function FallbackCat({ fat }: { fat: number }) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, filter: "drop-shadow(0 3px 5px rgba(26,22,19,0.12))" }}>
      ...
      <div
        style={{
          position: "absolute", left: "50%", bottom: 14, transform: "translateX(-50%)", width: bodyW, height: bodyH,
          borderRadius: `${lerp(46, 49, f)}% ${lerp(46, 49, f)}% 44% 44% / ${lerp(40, 50, f)}% ${lerp(40, 50, f)}% 48% 48%`,
          background: WHITE, boxShadow: `inset 0 -14px 22px rgba(110,116,130,0.16), inset 0 10px 16px rgba(255,255,255,0.7), 0 0 0 2.5px ${SHADE}`, zIndex: 1,
        }}
      >
